...
function EditProductForm({ product,onClose }) {
  ...
  const [selectedFile, setSelectedFile] = useState(product.name);
  ...
  const handleChange = (event) => {
    ...
    for (const element of files) {
      const file = element;
      // Do something with the file, like upload it or display its metadata
  ...
  return (<div>
    ...
    <div className="flex justify-between w-full">
      <div className="mt-5 mx-5 p-5 bg-offwhite w-3/5  shadow-md rounded-2xl h-120"style={{backgroundColor: '#fffffb'}}>
      ...
      <div className="mx-5 my-5 p-5 bg-offwhite h-80 shadow-md rounded-2xl" style={{backgroundColor: '#fffffb'}}>
  ...
  <div id="form-file-upload" >
    ...
    <label id="label-file-upload" htmlFor="input-file-upload">
      <div>
        ...
        { <p className="font-inter font-normal text-xs leading-4 leading-trim-cap text-gray-400">{selectedFiles} selected</p>}
    ...
    <div className="flex justify-between w-full">
  <div className="mt-5 ml-5 mr-12 p-5 bg-offwhite w-3/5 shadow-md rounded-2xl h-3/5" style={{backgroundColor: '#fffffb'}}>
    
    <div class="flex my-2">
    <label className="font-inter font-bold text-base leading-5 leading-trim-cap text-gray-700 ">Stock </label>
      ...
      />
       <label className="font-inter font-bold text-base leading-5 leading-trim-cap text-gray-700 ">Price</label>
  ...
  <div className="mr-5 ml-12 my-5 p-5 bg-offwhite shadow-md rounded-2xl "  style={{flex: 1,backgroundColor: '#fffffb'}}>
  ...
  <div class="flex items-center">
    <input type="radio"  name="visibility" id="Published" className="mr-2 outline-none " value={inStock} checked={inStock === true} onChange={handleVisibilityChange}/>
...
">Scheduled</label>
        ...
        <button className="btn bg-greey rounded-full p-3 m-3 border-none" onClick={onClose}>Cancel</button>
          <button className="bg-primary rounded-full p-3 m-3" onClick={handleSubmit}>Edit Product</button>
          </div>
