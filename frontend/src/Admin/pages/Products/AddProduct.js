...
const handleAddProduct = (event) => {
  ...
  axios.post("http://127.0.0.1:8000/homeLift/products/", formData, {
    headers: {
  ...
  .then((response) => {
    ...
    selectedFiles.forEach((file) => {
      ...
      axios.post(`http://127.0.0.1:8000/homeLift/products/${response.data.id}/images-create/`, 
      formData,
      {
        headers: {
          'content-type': 'multipart/form-data',
...
<div className="dashboard-content ">
  ...
  <label className=" outline-none font-inter font-bold text-base leading-5 leading-trim-cap text-gray-700 mb-5">Product Image</label>
    {!images ? <div id="form-file-upload" >
    <input type="file" id="input-file-upload" multiple={true} onChange={handleFileSelect} />
...
<input type="file" id="input-file-upload" multiple={true} onChange={handleFileSelect} />
<label id="label-file-upload" htmlFor="input-file-upload"><div>
...
<p className="upload-button">   Upload your product image.</p>}
  ...
  <div className="mr-5 ml-12 my-5 p-5 bg-white shadow-md rounded-2xl "  style={{flex: 1}}>
  ...
  <div class="flex items-center">
    <input type="radio" name="visibility" id="Published" className="mr-2 outline-none " value={in_stock}/>
...
<Link className="btn bg-greey rounded-full p-3 m-3 border-none" to='/products'>Cancel</Link>
  <button className="bg-teal-500 rounded-full p-3 m-3" type="submit" onClick={handleAddProduct}>Add Product</button>
